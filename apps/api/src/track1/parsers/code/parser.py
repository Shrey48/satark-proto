"""
SATARK Layer 1 — Python Code Parser (A1)
...
E_invoke is only created by tree-sitter (AST-exact call detection).
Reason: regex cannot reliably distinguish self.fn() method calls from fn() direct calls.
"""
import re
...
# ── Tree-sitter (E_contain + E_invoke) ────────────────────────────────────────

def _parse_with_treesitter(tree, content: str, file_path: str, asset_id: str, fragment: GraphFragment) -> GraphFragment:
    lines = content.split("\n")
    ...
    _walk_ts(tree.walk(), content, file_path, asset_id, fragment, file_id, None, None, func_registry)
...
def _walk_ts(cursor, content: str, file_path: str, asset_id: str,
    ...
    if node.type == "class_definition":
        ...
        fragment.nodes.append(KGNode(
            ...
            metadata=NodeMetadata(semantic_summary=f"Python class {class_name}", resolved_by="deterministic"),
            org_id=ORG_ID,
        ))
    ...
    if node.type == "function_definition":
        ...
        # Recurse into body with this func_id as current_func_id
        if cursor.goto_first_child():
            while True:
                _walk_ts(cursor, content, file_path, asset_id, fragment,
    ...
    # E_invoke: only from `call` nodes where callee is a bare identifier (NOT attribute)
    # self.fn() → callee is `attribute` → NOT matched
    # fn()      → callee is `identifier` → matched
    if node.type == "call" and current_func_id:
        ...
        if func_node and func_node.type == "identifier":
            ...
            if called_name in func_registry:
                ...
                if called_id != current_func_id:
                    # Deduplicate edges within this fragment
                    existing = any(
        ...
        return  # Don't recurse into call args (avoids double-counting)
...
# ── Regex fallback (E_contain only — NO E_invoke) ─────────────────────────────

def _parse_with_regex(content: str, file_path: str, asset_id: str, fragment: GraphFragment) -> GraphFragment:
    """
    Regex parser: produces E_contain edges only.
    E_invoke is NOT produced because regex cannot reliably distinguish
    direct function calls from method calls (self.fn() vs fn()).
    """
    lines = content.split("\n")
    file_id = _make_entity_id(file_path, "file", file_path.split("/")[-1])
    fragment.nodes.append(KGNode(
    ...
    while i < len(lines):
        ...
        if cm:
            ...
            fragment.nodes.append(KGNode(
                ...
                metadata=NodeMetadata(semantic_summary=f"Python class {cname}", resolved_by="deterministic"),
                org_id=ORG_ID,
            ))
        ...
        if fm:
            ...
            # Peek at first few lines of body for taint detection only
            body_peek  = "\n".join(lines[i+1:min(i+30, len(lines))])
            taint_class = _detect_taint_class(body_peek)

            parent_id  = (_make_entity_id(file_path, "class", current_class)
                          if is_method else file_id)
            func_id    = _make_entity_id(file_path, "function", func_name)
            ...
            fragment.edges.append(KGEdge(from_entity_id=parent_id, to_entity_id=func_id,
                                         edge_type="E_contain", source_asset_ids=[asset_id]))
            # NOTE: NO E_invoke edges created here — regex cannot reliably detect calls
            pending_decorators = []
            i += 1
            continue
    ...
    logger.info("code_parsed_regex", file=file_path, nodes=len(fragment.nodes), edges=len(fragment.edges))
    return fragment
